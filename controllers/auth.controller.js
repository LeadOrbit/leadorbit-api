...
exports.sendOtp = async (req, res) => {
  ...
  await sendEmail({
    to: email, subject: "Email Verification", html: `<p>Hello ${username || ""},</p>
...
exports.verifyOtp = async (req, res) => {
  ...
  const admins = ["vishal.singh@jaiinfoway.com", "addy@jaiinfoway.com", "jai@jaiinfoway.com"]
  const token = jwt.sign({ email, role: admins.includes(email) ? "admin" : "user" }, JWT_SECRET, { expiresIn: "7d" });



  res.status(200).send({ success: true, message: "OTP verified", token: token, username: user.username, email, role: admins.includes(email) ? "admin" : "user" });
};
