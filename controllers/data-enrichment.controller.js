...
const enrichmentWithApollo = async (req, res) => {
  try {
    ...
    res.status(200).send({
      success: true,
      message:
        "Enrichment process started.Go to Jobs tab to track progress of the task.",
    });
    ...
    setImmediate(async () => {
      try {
        ...
        let [contactListEntry] = await db.ContactList.findOrCreate({
          ...
          defaults: {
            name: name.trim().length
              ? name.trim()
              : `Contacts for list ${companyListId}`,
            source,
            status: "Running",
            contacts: [],
            dataSourceListId: companyListId,
        ...
        while (lastProcessedRowNumber < rows.length) {
          ...
          const peopleInfo = await processBatch(
            cleanBatch,
            selectedList.source,
          ...
          if (!peopleInfo.length) {
            ...
            if (lastProcessedRowNumber >= rows.length) {
              ...
              console.log(
                "Final batch with empty peopleInfo. Marked ContactList as Completed.",
              );
            }
          ...
          const existingProfiles = Array.isArray(contactListEntry.contacts)
            ...
            : [];
          const updatedProfilesMap = new Map(
  ...
  try {
    ...
    if (!enrichmentJob) {
      return res
        ...
        .json({
          success: false,
          message: "Enrichment job not found or access denied",
        });
    }
...
const saveOrUpdateCredentials = async (req, res) => {
  ...
  const source = credentialsData.source;
  try {
...
module.exports = {
  enrichmentWithApollo,
  getEnrichmentJobsList,
  getEnrichedData,
  deleteEnrichedDataList,
