...
const addTrackingPixel = (body, trackingId, recipientId) => {
    const baseUrl = `${process.env.BACKEND_URL_PROD}/api/v1`;
    ...
    const unsubscribeLink = `<p style="font-size: 12px; color: #888; margin-top: 20px;">
        If you no longer wish to receive emails, <a href="${process.env.FRONTEND_URL_PROD}/unsubscribe/${recipientId}/${trackingId}" target="_blank">unsubscribe here</a>.
    </p>`;
    ...
    if (body.includes("</body>")) {
        return body.replace("</body>", `${inlineStyles}${trackingElements}${unsubscribeLink}${manualTrackingInfo}</body>`);
    } else if (body.includes("</html>")) {
        return body.replace("</html>", `${inlineStyles}${trackingElements}${unsubscribeLink}${manualTrackingInfo}</html>`);
...
const processScheduledCampaigns = async () => {
    console.log(`[${new Date().toISOString()}] 🔍 Checking for pending scheduled campaigns...`);
    ...
    try {
        ...
        if (scheduledSteps.length === 0) {
            console.log(`[${new Date().toISOString()}] ✅ No pending campaigns to process.`);
            return;
        }
        ...
        for (const step of scheduledSteps) {
            ...
            if (!recipient?.email) {
                console.warn(`⚠️ Missing recipient email in step ID ${step.id}, skipping...`);
                continue;
            }

            const recipientEmail = recipient.email;
            console.log(`📤 Preparing to send to: ${recipientEmail} for Campaign ID: ${campaign.id}`);
            ...
            if (senders.length === 0) {
                console.warn(`⚠️ No senders available in sender list ${step.senderListId}, skipping...`);
                continue;
            }
            ...
            if (previousTracker?.senderEmail) {
                ...
                if (sender) {
                    console.log(`🔁 Reusing sender ${sender.email} for ${recipientEmail}`);
                } else {
                    console.warn(`⚠️ Previous sender ${previousTracker.senderEmail} not in current sender list. Falling back to round-robin.`);
                }
            }
            ...
            if (!template?.content) {
                console.warn(`⚠️ Missing template content for step ${step.id}, skipping...`);
                continue;
            }

            const placeHolders = ["{{name}}", "{{company}}", "{{designation}}", "{{industry}}", "{{city}}", "{{state}}"];
            let emailContent = template.content;

            placeHolders.forEach((placeholder) => {
                switch (placeholder) {
                    case "{{name}}":
                        emailContent = emailContent.replace(placeholder, recipient.name || 'User');
                        break;
                    case "{{company}}":
                        emailContent = emailContent.replace(placeholder, recipient.organisation.name || '');
                        break;
                    case "{{designation}}":
                        emailContent = emailContent.replace(placeholder, recipient.organisation.title || '');
                        break;
                    case "{{industry}}":
                        emailContent = emailContent.replace(placeholder, recipient.organisation.industry || '');
                        break;
                    case "{{country}}":
                        emailContent = emailContent.replace(placeholder, recipient.country || '');
                        break;
                    case "{{city}}":
                        emailContent = emailContent.replace(placeholder, recipient.city || '');
                        break;
                    case "{{state}}":
                        emailContent = emailContent.replace(placeholder, recipient.state || '');
                        break;
                    default:
                        break;
                }
            ...
            try {
                ...
                await sendEmail(sender.email, recipientEmail, template.subject, emailContent, replyTo);
                ...
                console.error(`❌ Failed to send email to ${recipientEmail} using ${sender.email}:`, err.message);
                step.status = 'failed';
                await step.save();
            }
    ...
    console.log(`[${new Date().toISOString()}] 🏁 Finished processing scheduled campaigns.`);
};
